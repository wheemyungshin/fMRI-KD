...
def load_ckp(model):
  checkpoint_fpath = SAVEPATH+'teacher34_model_weight_best.pt'
...
def main(brain_teacher_target=None):
    ...
    if teacher_model is not None:
        ...
        teacher_model = load_ckp(teacher_model)
    ...
    for sbj, roi, feat in product(subjects, rois, features):
        ...
        for epoch in range(start_epoch, EPOCHS):
            ...
            last_top1_acc = train(train_loader, epoch, model, model_fmri, optimizer, criterion, teacher_model=None)#, brain_teacher=noise_teacher)
            ...
            start_time = time.time()
            acc1_valid = validate(val_loader, model, model_fmri, criterion)
            elapsed_time = time.time() - start_time
...
def train(train_loader, epoch, model, model_fmri, optimizer, criterion, teacher_model=None, brain_teacher=None):
...
def validate(val_loader, model, model_fmri, criterion):
    batch_time = AverageMeter("Time", ":6.3f")
    losses = AverageMeter("Loss", ":.4e")
    top1 = AverageMeter("Acc@1", ":6.2f")
    top5 = AverageMeter("Acc@5", ":6.2f")
...
if __name__ == "__main__":
    result_list = []
    for i in range(5):
        best_acc1 = main(brain_teacher_target='soft_label.npy')
        result_list.append(best_acc1)
    print(result_list)
