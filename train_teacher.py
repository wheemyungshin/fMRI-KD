...
def save_ckpt(state, is_best, sbj):
  f_path = SAVEPATH+'model1d_'+sbj+'_weight_latest.pt'
  torch.save(state, f_path)
  if is_best:
    best_fpath = SAVEPATH+'model1d_'+sbj+'_weight_best.pt'
    shutil.copyfile(f_path, best_fpath)

def load_ckp(model):
  checkpoint_fpath = SAVEPATH+'model1d_weight_latest.pt'
...
def main(brain_teacher_target=None):
    ...
    for sbj, roi, feat in product(subjects, rois, features):
        ...
        for epoch in range(start_epoch, EPOCHS):
            ...
            last_top1_acc = train(train_loader, epoch, model, optimizer, criterion, feature_kd_criterion, adap_layers, teacher_model=None, brain_teacher=None)
            ...
            start_time = time.time()
            acc1_valid = validate(val_loader, model, criterion, feature_kd_criterion)
            elapsed_time = time.time() - start_time
            ...
            save_ckpt(checkpoint, is_train_best, sbj)
        ...
        print(f"Number of parameters: {pytorch_total_params}")
        print(brain_teacher_target)
        bests[(sbj, roi)] = acc1_valid
        print(bests)

    return bests


def train(train_loader, epoch, model, optimizer, criterion, feature_kd_criterion, adap_layers, teacher_model=None, brain_teacher=None):
...
def validate(val_loader, model, criterion, feature_kd_criterion):
    batch_time = AverageMeter("Time", ":6.3f")
    losses = AverageMeter("Loss", ":.4e")
    top1 = AverageMeter("Acc@1", ":6.2f")
    top5 = AverageMeter("Acc@5", ":6.2f")
